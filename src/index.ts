    ...
    <script>
        async function askAI() {
            ...
            try {
                ...
                box.innerText = text;
                const logEntry = \`Q: \${query} <br> A: \${text}\`;
                await fetch('/remember/' + encodeURIComponent(logEntry));
            } catch (e) {
...
app.get("/ai-test", async (c) => {
  const userPrompt = c.req.query("prompt") as string || "Explain why Cloudflare is a good company to work for in 2 sentences.";
...
app.get("/view-history", async (c) => {
  ...
  const historyList = data.history.length > 0 
    ...
    <div style="margin-top: 20px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 15px;">
        <a href="/clear-history" style="font-size: 0.75rem; color: #ef4444; text-decoration: none; border: 1px solid #ef4444; padding: 6px 12px; border-radius: 8px; display: inline-block;">
            Clear All Logs
        </a>
    </div>
  `;

  return c.html(renderPage("Memory Logs", content));
});
...
app.get("/clear-history", async (c) => {
    ...
    // Call the DO with the clear action
    await obj.fetch("http://do/clear?action=clear");
    
    return c.html(renderPage("Memory Logs", "Memory has been successfully cleared."));
});
